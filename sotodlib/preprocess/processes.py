...
class EstimateHWPSS(_Preprocess):
    ...
    def gen_metric(cls, meta, proc_aman):
        ...
        for ds in np.unique(meta.det_info.detset):
            ...
            perc = np.percentile(coeff[nonzero], cls._influx_percentiles, axis=0)
            mean = coeff[nonzero].mean(axis=0)

            tags_base = {
                k: meta.det_info[k][subset[0]] for k in tag_keys if k in meta.det_info
            }
            ...
            for pi, p in enumerate(cls._influx_percentiles):
                for l in proc_aman.hwpss_stats.modes.vals:
