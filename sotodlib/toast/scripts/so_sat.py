...
"""
This script runs a SAT mapmaking with splits on either real or
a single instance of simulated data.
...
Or you can dump a config file with all the default values with:
    ...
    toast_so_sat --default_toml config.toml
...
def reduce_data(job, otherargs, runargs, data):
    ...
    # Set the mapmaker to use for splits
    job_ops = job.operators
    if job_ops.mapmaker.enabled:
        job_ops.splits.mapmaker = job_ops.mapmaker
    elif job_ops.filterbin.enabled:
        job_ops.splits.mapmaker = job_ops.filterbin
    else:
        msg = "No mapmaker is enabled!"
        raise RuntimeError(msg)

    # Loop over splits, saving map products in memory
    wrk.split(job, otherargs, runargs, data)

    # Write out products
    job_ops.splits.write_splits(data)
